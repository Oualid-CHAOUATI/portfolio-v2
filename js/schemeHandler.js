class SchemeHandler {
  constructor() {
    this.localStorageKey = "walid-portfolio-scheme";
    this.initDom();
    this.getSchemeFromBrowser();
    this.handleSchemeToggle();
  }

  initDom() {
    this.$schemeInput = document.querySelector("#scheme-input");
  ...
  >
    ...
    const $schemeInputLabel = document.querySelector("#scheme-input-label");

    const $svgsWrapper = document.createElement("div");
    $svgsWrapper.id = "svgs-wrapper";
    $schemeInputLabel.append($svgsWrapper);
    $svgsWrapper.innerHTML += sunIcon;
    $svgsWrapper.innerHTML += moonIcon;
  }

  getSchemeFromBrowser() {
    const scheme = localStorage.getItem(this.localStorageKey);
    if (scheme) {
      if (scheme == "dark") this.$schemeInput.checked = true;
    } else {
      const prefersDarkScheme = window.matchMedia("prefers-color-scheme:dark");

      if (prefersDarkScheme) {
        this.$schemeInput.checked = true;
      }
    }
  }
  handleSchemeToggle() {
    this.$schemeInput.addEventListener("change", () => {
      localStorage.setItem(
        this.localStorageKey,
        this.$schemeInput.checked ? "dark" : "light"
      );
    });
  }
}
